...
import {
  claimPurchaseWelcomeEmail,
  releasePurchaseWelcomeEmailClaim,
} from '@/lib/data';
...
function buildWelcomeEmailContent(productName: string) {
  const subject = `Welcome to ${productName} — access unlocked`;

  const text = [
    `Thanks for signing up for ${productName}!`,
    '',
    'Your purchase is complete and your access is now unlocked.',
  ...
  const html = [
    ...
    `<p style="margin: 0 0 12px;">Thanks for signing up! Your purchase is complete and your access is now unlocked.</p>`,
...
/**
 * Send a "thank you for signing up / access granted" welcome email after a
 * completed purchase.
 */
export async function sendPurchaseWelcomeEmail({
  ...
  if (!twilioEmailConfigured) {
    console.warn('Skipping purchase welcome email: Twilio email is not configured.');
    return false;
  }

  if (!to) {
    console.warn('Skipping purchase welcome email: no recipient email address.');
    return false;
  }
  ...
  try {
    ...
    console.error('Failed to send purchase welcome email via Twilio:', error);
    return false;
  }
}

/**
 * Send the purchase welcome email exactly once, even though both the Stripe
 * webhook and the success page call this for the same purchase (and may race).
 */
export async function sendWelcomeEmailForPurchaseOnce({
  ...
  if (!twilioEmailConfigured || !to) {
    return sendPurchaseWelcomeEmail({ to, productName });
  }

  if (!databaseConfigured || !stripePaymentIntentId) {
    return sendPurchaseWelcomeEmail({ to, productName });
  }

  let claimed = false;
  try {
    claimed = await claimPurchaseWelcomeEmail(stripePaymentIntentId);
  } catch (error) {
    console.error('Failed to claim purchase welcome email; skipping send:', error);
    return false;
  }
  ...
  const sent = await sendPurchaseWelcomeEmail({ to, productName });

  if (!sent) {
    try {
      await releasePurchaseWelcomeEmailClaim(stripePaymentIntentId);
    } catch (error) {
